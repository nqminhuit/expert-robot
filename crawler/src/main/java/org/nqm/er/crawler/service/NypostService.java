...
import org.nqm.er.common.NypostSiteMap.NypostMonthSiteMap;
...
public class NypostService {
    ...
    public Optional<List<NypostSiteMap>> extractSitemapFromUrl(String url) {
                ...
                .map(doc -> doc.select("div.page__content.entry-content div.layout__grid div.layout__item")
                        ...
                        .map(el -> {
                            ...
                            var months = el.select("div.layout__inner div.date-link a")
                                    .stream()
                                    .map(ell -> new NypostMonthSiteMap(
                                            ErDateUtils.getMonth(ell.text()),
                                            ell.attr("href")))
                                    .toList();
                            return new NypostSiteMap(year, months);
                        })
                        .toList());
    }
}
